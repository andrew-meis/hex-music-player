...
import { toNumber, toTimestamp, toDate, toFloat } from './types';
...
const toAlbum = ($data: Prism<any>): Album => ({
  ...
  format: $data.get('Format', { quiet: true }).transform(toTagList).value,
  ...
  subformat: $data.get('Subformat', { quiet: true }).transform(toTagList).value,
